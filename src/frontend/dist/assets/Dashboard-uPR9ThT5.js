import { d as commonjsGlobal, g as getDefaultExportFromCjs, r as reactExports, e as clsx, R as React, u as useCompanySettings, f as useActor, j as jsxRuntimeExports, U as Users, A as ArrowLeftRight } from "./index-Bn6g6lYN.js";
import { C as Card, c as CardContent, a as CardHeader, b as CardTitle } from "./card-DLSHX_61.js";
import { T as Tabs, a as TabsList, b as TabsTrigger, c as TabsContent } from "./tabs-6qZQGtZM.js";
import { u as useExpenses, B as Banknote, T as TrendingUp, a as TrendingDown, C as Calendar } from "./useExpenses-BDSFr5VW.js";
import { u as useLocalCustomers } from "./useLocalCustomers-PRZwf9X2.js";
import { C as CircleAlert } from "./circle-alert-Df1NKVM9.js";
...
function formatDate(time2) {
  const ms = Number(time2 / 1000000n);
...
function getLast12Months() {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    result.push({
      year: d2.getFullYear(),
      month: d2.getMonth(),
      label: `${BANGLA_MONTHS[d2.getMonth()]} ${d2.getFullYear()}`
    });
  }
  return result;
}
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          ...
          payload.map((entry) => /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { style: { color: entry.color, margin: "2px 0" }, children: [
            entry.name,
            ": ৳",
            Number(entry.value).toLocaleString("bn-BD")
          ] }, entry.name))
        ]
      }
    );
  ...
  if (active && payload && payload.length) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { style: { fontWeight: 600, color: payload[0].payload.fill }, children: payload[0].name }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { style: { color: "var(--foreground)" }, children: [
            "৳",
...
function Dashboard({
  ...
  const [debtSummary, setDebtSummary] = reactExports.useState({
    totalReceivables: 0,
    totalPayables: 0
  });
  reactExports.useEffect(() => {
    if (isAdmin && actor) {
      actor.getDebtSummary().then((s2) => setDebtSummary(s2)).catch(() => {
      });
    }
  }, [isAdmin, actor]);
  const totalCustomers = customers.length;
  const expectedMonthly = customers.reduce((sum, c2) => sum + c2.monthlyFee, 0);
  const totalCommission = customers.reduce((sum, c2) => {
    const periods = completedPeriods(c2.connectionDate);
    return sum + periods * Math.round(c2.monthlyFee * (c2.commissionPercent ?? 30) / 100);
  }, 0);
  const totalConnectionFeeDue = customers.reduce(
    (sum, c2) => sum + c2.connectionFeeDue,
    0
  );
  const totalConnectionFeeCash = customers.reduce(
    (sum, c2) => sum + c2.connectionFeeCash,
    0
  );
  const totalExpenses = expenses.reduce((sum, e3) => sum + e3.amount, 0);
  const recentCustomers = [...customers].sort((a2, b2) => Number(b2.connectionDate - a2.connectionDate)).slice(0, 5);
  const last12 = getLast12Months();
  const monthlyData = last12.map(({ year, month, label }) => {
    const endOfMonth = new Date(year, month + 1, 0, 23, 59, 59).getTime();
    const activeCustomers = customers.filter(
      (c2) => Number(c2.connectionDate / 1000000n) <= endOfMonth
    );
    const income = activeCustomers.reduce((s2, c2) => s2 + c2.monthlyFee, 0);
    const commission = activeCustomers.reduce(
      (s2, c2) => s2 + Math.round(c2.monthlyFee * (c2.commissionPercent ?? 30) / 100),
      0
    );
    const monthKey = `${year}-${String(month + 1).padStart(2, "0")}`;
    const expense = expenses.filter((e3) => {
      var _a;
      return (_a = e3.date) == null ? void 0 : _a.startsWith(monthKey);
    }).reduce((s2, e3) => s2 + e3.amount, 0);
    return { label, income, commission, expense };
  });
  const yearsSet = /* @__PURE__ */ new Set();
  for (const c2 of customers) {
    const y2 = new Date(Number(c2.connectionDate / 1000000n)).getFullYear();
    yearsSet.add(y2);
  }
  for (const e3 of expenses) {
    if (e3.date) yearsSet.add(Number(e3.date.slice(0, 4)));
  }
  const years = Array.from(yearsSet).sort();
  if (years.length === 0) years.push((/* @__PURE__ */ new Date()).getFullYear());
  const yearlyData = years.map((year) => {
    let income = 0;
    let connectionFeeInYear = 0;
    for (const c2 of customers) {
      const connMs = Number(c2.connectionDate / 1000000n);
      const connDate = new Date(connMs);
      const connYear = connDate.getFullYear();
      if (connYear === year) connectionFeeInYear += c2.connectionFeeCash;
      const startMonth = connYear === year ? connDate.getMonth() : 0;
      const endMonth = year < (/* @__PURE__ */ new Date()).getFullYear() ? 11 : (/* @__PURE__ */ new Date()).getMonth();
      if (connYear <= year) {
        income += c2.monthlyFee * Math.max(0, endMonth - startMonth + 1);
      }
    }
    const expense = expenses.filter((e3) => {
      var _a;
      return (_a = e3.date) == null ? void 0 : _a.startsWith(String(year));
    }).reduce((s2, e3) => s2 + e3.amount, 0);
    return {
      label: String(year),
  ...
  for (const e3 of expenses) {
    expenseByCategoryMap[e3.category] = (expenseByCategoryMap[e3.category] ?? 0) + e3.amount;
  }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-6", "data-ocid": "dashboard.page", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4", children: [
        ...
        Card,
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "pt-5 pb-5", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground font-medium mb-2", children: "মোট গ্রাহক" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-3xl font-bold text-foreground", children: totalCustomers }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mt-1", children: "জন" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-2.5 rounded-xl bg-primary/10", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Users, { className: "w-5 h-5 text-primary" }) })
          ] }) })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Card,
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "pt-5 pb-5", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground font-medium mb-2", children: "সংযোগ বাবদ আয়" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-3xl font-bold text-teal-600", children: formatCurrency(totalConnectionFeeCash) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mt-1", children: "আদায়কৃত সংযোগ ফি" })
            ] }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Card,
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "pt-5 pb-5", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground font-medium mb-2", children: "কমিশন বাবদ আয়" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-3xl font-bold text-success", children: formatCurrency(totalCommission) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mt-1", children: "সংযোগ শুরু থেকে" })
            ] }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Card,
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "pt-5 pb-5", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground font-medium mb-2", children: "মোট ব্যয়" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-3xl font-bold text-destructive", children: formatCurrency(totalExpenses) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mt-1", children: "সকল খাত মিলিয়ে" })
            ] }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Card,
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "pt-5 pb-5", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground font-medium mb-2", children: "এই মাসের প্রত্যাশিত আয়" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-2xl font-bold text-foreground", children: formatCurrency(expectedMonthly) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mt-1", children: "প্রতি মাসে প্রত্যাশিত" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-2.5 rounded-xl bg-primary/10", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Calendar, { className: "w-5 h-5 text-primary" }) })
          ] }) })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Card,
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "pt-5 pb-5", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground font-medium mb-2", children: "মোট বকেয়া সংযোগ ফি" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-2xl font-bold text-destructive", children: formatCurrency(totalConnectionFeeDue) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mt-1", children: "অপরিশোধিত বকেয়া" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-2.5 rounded-xl bg-destructive/10", children: /* @__PURE__ */ jsxRuntimeExports.jsx(CircleAlert, { className: "w-5 h-5 text-destructive" }) })
          ] }) })
        }
      )
    ] }),
    isAdmin && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Card,
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "pt-5 pb-5", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground font-medium mb-2", children: "মোট প্রাপ্য বকেয়া" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-2xl font-bold text-teal-600", children: formatCurrency(debtSummary.totalReceivables) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mt-1", children: "সংযোগ ফি ও কমিশন" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-2.5 rounded-xl bg-teal-500/10", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ArrowLeftRight, { className: "w-5 h-5 text-teal-600" }) })
          ] }) })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        Card,
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "pt-5 pb-5", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground font-medium mb-2", children: "মোট প্রদেয় বকেয়া" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-2xl font-bold text-orange-600", children: formatCurrency(debtSummary.totalPayables) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mt-1", children: "টেকনিশিয়ান, হোলসেলার, অগ্রিম" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-2.5 rounded-xl bg-orange-500/10", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ArrowLeftRight, { className: "w-5 h-5 text-orange-600" }) })
          ] }) })
        }
      )
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-xl border border-border bg-muted/30 px-4 py-3 flex flex-col sm:flex-row gap-3 sm:gap-6 items-start sm:items-center", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs font-semibold text-muted-foreground shrink-0", children: "কর্তৃপক্ষ:" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col sm:flex-row gap-2 sm:gap-6 text-xs text-foreground", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-semibold", children: "পরিচালক:" }),
          " ",
          settings.directorName
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-semibold", children: "টেকনিশিয়ান:" }),
          " ",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      Card,
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(CardHeader, { className: "pb-2", children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardTitle, { className: "text-base font-semibold", children: "আয়‑ব্যয় বিশ্লেষণ" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Tabs, { defaultValue: "monthly", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsList, { className: "mb-4 bg-gray-100 dark:bg-gray-800 p-1 gap-1", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                TabsTrigger,
                {
                  value: "monthly",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsContent, { value: "monthly", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mb-3", children: "গত ১২ মাসের মাসিক আয় ও ব্যয়" }),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsContent, { value: "yearly", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mb-3", children: "বছরভিত্তিক মোট আয় ও ব্যয়" }),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsContent, { value: "category", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mb-4", children: "খাতভিত্তিক আয় ও ব্যয়ের চিত্র" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-8", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm font-semibold text-center mb-3 text-success", children: "আয়ের উৎস" }),
                  incomeBySource.every((d2) => d2.value === 0) ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-center text-muted-foreground text-sm py-8", children: "কোনো আয় নেই" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 240, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(PieChart, { children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      Pie,
                      {
                        data: incomeBySource,
                        ...
                        labelLine: false,
                        children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx(Cell, { fill: "#22C55E" }, "c0"),
                          /* @__PURE__ */ jsxRuntimeExports.jsx(Cell, { fill: "#14B8A6" }, "c1")
                        ]
                      }
                    ),
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm font-semibold text-center mb-3 text-destructive", children: "ব্যয়ের খাত" }),
                  expenseByCategory.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-center text-muted-foreground text-sm py-8", children: "কোনো ব্যয় নেই" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 240, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(PieChart, { children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      Pie,
                      {
                        ...
                        label: ({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`,
                        labelLine: false,
                        children: expenseByCategory.map((cat, idx) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                          Cell,
                          {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      Card,
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(CardHeader, { className: "pb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardTitle, { className: "text-base font-semibold", children: "সাম্প্রতিক গ্রাহক" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "overflow-x-auto", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "w-full text-sm", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: "border-b border-border", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left py-2.5 px-3 text-xs font-semibold text-muted-foreground", children: "নাম" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left py-2.5 px-3 text-xs font-semibold text-muted-foreground", children: "ফোন" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left py-2.5 px-3 text-xs font-semibold text-muted-foreground", children: "গ্রাম" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left py-2.5 px-3 text-xs font-semibold text-muted-foreground", children: "প্যাকেজ" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left py-2.5 px-3 text-xs font-semibold text-muted-foreground", children: "যোগদান" })
              ] }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: recentCustomers.map((c2, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "tr",
                {
                  className: "border-b border-border last:border-0 hover:bg-muted/30",
                  "data-ocid": `dashboard.recent_customers.item.${i + 1}`,
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "py-2.5 px-3 font-medium", children: c2.username }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "py-2.5 px-3 text-muted-foreground", children: c2.phone }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "py-2.5 px-3 text-muted-foreground", children: c2.village }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { className: "py-2.5 px-3 text-muted-foreground", children: [
                      "৳",
                      c2.monthlyFee,
                      "/মাস"
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "py-2.5 px-3 text-muted-foreground", children: formatDate(c2.connectionDate) })
                  ]
                },
                c2.id.toString()
              )) })
            ] }),
            recentCustomers.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(
              "p",
              {
